...
func TestGetPubkeyFromPrivkey(t *testing.T) {
	...
	recoverPubkey := GetPubkeyFromPrivkey(privKey)

	if !bytes.Equal(pubKey, recoverPubkey) {
		t.Errorf("pubkey mismatch: want: %x have: %x", pubKey, recoverPubkey)
	}
}

func TestGetCompressedPubkeyFromPrivkey(t *testing.T) {
	pubKey, privKey := generateKeyPair()
	X, Y := elliptic.Unmarshal(S256(),pubKey)
	compressedPubkey := CompressPubkey(X,Y)

	recoverPubkey := GetCompressedPubkeyFromPrivkey(privKey)

	if !bytes.Equal(compressedPubkey, recoverPubkey) {
		t.Errorf("pubkey mismatch: want: %x have: %x", compressedPubkey, recoverPubkey)
	}
}
