...
// Get the pubKey from the privKey
func GetPubkeyFromPrivkey(priv []byte) []byte {
	var seckey_ptr *C.uchar = (*C.uchar)(unsafe.Pointer(&priv[0]))
	...
	if ret != C.int(1) {
		return GetPubkeyFromPrivkey(priv) // invalid secret, try again
	}

	var output_len C.size_t = 65

	_, _ = pubkey65_ptr, output_len
	...
	C.secp256k1_ec_pubkey_serialize( // always returns 1
		...
		&output_len,
		pubkey64_ptr,
		C.SECP256K1_EC_UNCOMPRESSED, // SECP256K1_EC_COMPRESSED
	)
...
// Get the compressed pubKey from the privKey
func GetCompressedPubkeyFromPrivkey(priv []byte) []byte {
	var seckey_ptr *C.uchar = (*C.uchar)(unsafe.Pointer(&priv[0]))
	var pubkey64 []byte = make([]byte, 64) // secp256k1_pubkey
	var pubkey65 []byte = make([]byte, 33) // 65 byte uncompressed pubkey
	pubkey64_ptr := (*C.secp256k1_pubkey)(unsafe.Pointer(&pubkey64[0]))
	pubkey65_ptr := (*C.uchar)(unsafe.Pointer(&pubkey65[0]))
	...
	if ret != C.int(1) {
		return GetCompressedPubkeyFromPrivkey(priv) // invalid secret, try again
	}

	var output_len C.size_t = 33
	...
	C.secp256k1_ec_pubkey_serialize( // always returns 1
		...
		&output_len,
		pubkey64_ptr,
		C.SECP256K1_EC_COMPRESSED, // SECP256K1_EC_COMPRESSED
	)
